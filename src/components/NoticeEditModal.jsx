    ...
    return (
        <div onClick={() => {
            ...
            <form onClick={(e) => e.stopPropagation()} className="w-[30%] h-screen overflow-auto fixed top-0 right-0 transition-all duration-300 bg-white" style={{ right: modal ? "0" : "-200%" }}>
                <div className="flex justify-between text-xl p-5 border-b-2"><h1>Update notice</h1> <button type="button" onClick={() => {
                    setModal(false)
                ...
                <div className="flex flex-col gap-2 px-5 py-7">
                    ...
                    <button disabled={isLoading ? true : false} onClick={(e) => updateHandler(e)} className="w-fit px-6 py-1 mt-8 border-2 border-cyan-600 rounded-lg hover:text-white hover:bg-cyan-600 transition-all duration-300">{isLoading ? "Loading..." : "Save"}</button>
                </div>
            </form>
        </div>
    )
